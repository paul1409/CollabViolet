...
/**
 * A service for storing and loading user preferences. This service uses either
 * the standard Java preferences API or the WebStart persistence service
 * ("muffins").
 */
public abstract class PreferencesService {
  /**
   * Gets an instance of the service, suitable for the package of the given
   * class.
   * @param appClass the main application class (only the package name is used
   *          as the path to app-specific preferences storage)
  ...
  public static PreferencesService getInstance(Class appClass) {
    ...
    try {
      ...
      service = (PreferencesService) Class.forName("com.horstmann.violet.framework.JNLPPreferencesService")
          .newInstance();
      return service;
    }
