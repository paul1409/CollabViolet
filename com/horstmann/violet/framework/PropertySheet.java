...
public class PropertySheet extends JPanel {
  ...
  public Component getEditorComponent(final PropertyEditor editor) {
    ...
    if (editor.supportsCustomEditor()) {
      ...
      /*
       * 
       * // Make a button that pops up the custom editor final JButton button =
       * new JButton(); // if the editor is paintable, have it paint an icon if
       * (editor.isPaintable()) { button.setIcon(new Icon() { public int
       * getIconWidth() { return WIDTH - 8; } public int getIconHeight() {
       * return HEIGHT - 8; }
       * 
       * public void paintIcon(Component c, Graphics g, int x, int y) {
       * g.translate(x, y); Rectangle r = new Rectangle(0, 0, getIconWidth(),
       * getIconHeight()); Color oldColor = g.getColor();
       * g.setColor(Color.BLACK); editor.paintValue(g, r); g.setColor(oldColor);
       * g.translate(-x, -y); } }); } else button.setText(buttonText(text)); //
       * pop up custom editor when button is clicked
       * button.addActionListener(new ActionListener() { public void
       * actionPerformed(ActionEvent event) { final Component customEditor =
       * editor.getCustomEditor();
       * 
       * JOptionPane.showMessageDialog(parent, customEditor);
       * 
       * // This should really be showInternalMessageDialog, // but then you get
       * awful focus behavior with JDK 5.0 // (i.e. the property sheet retains
       * focus). In // particular, the color dialog never works.
       * 
       * if (editor.isPaintable()) button.repaint(); else
       * button.setText(buttonText(editor.getAsText())); } }); return button;
       */
    } else if (tags != null) {
  ...
  private static String buttonText(String text) {
    ...
    if (text.length() > MAX_TEXT_LENGTH) return text.substring(0, MAX_TEXT_LENGTH) + "...";
    return text;
  }
