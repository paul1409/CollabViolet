...
public class GraphPanel extends JPanel {
  ...
  public GraphPanel(ToolBar aToolBar) {
    ...
    addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent event) {
        ...
        if (event.getClickCount() > 1 || (event.getModifiers() & InputEvent.BUTTON1_MASK) == 0)
        // double/right-click
        {
          if (e != null) {
            setSelectedItem(e);
        ...
        } else if (tool == null) // select
        {
          if (e != null) {
            ...
            if (isCtrl) addSelectedItem(n);
            else if (!selectedItems.contains(n)) setSelectedItem(n);
            dragMode = DRAG_MOVE;
          } else {
  ...
  public void editSelected() {
    ...
    if (lastSelected == null) {
      if (selectedItems.size() == 1) edited = selectedItems.iterator().next();
      else return;
    }
  ...
  public void paintComponent(Graphics g) {
    ...
    if (!hideGrid) grid.draw(g2, new Rectangle2D.Double(0, 0, Math.max(bounds.getMaxX() / zoom, graphBounds.getMaxX()),
   ...
   * multiplied by sqrt(2) for each positive stem or divided by sqrt(2) for each
   * negative step.
   * @param steps the number of steps by which to change the zoom. A positive
   *          value zooms in, a negative value zooms out.
   */
