...
public class SequenceDiagramGraph extends Graph {
  ...
  public void removeEdge(Edge e) {
    super.removeEdge(e);
    if (e instanceof CallEdge && e.getEnd().getChildren().size() == 0) removeNode(e.getEnd());
  }
  ...
  public void layout(Graphics2D g2, Grid grid) {
    ...
    while (iter.hasNext()) {
      ...
      if (n instanceof CallNode && n.getParent() == null) topLevelCalls.add(n);
      else if (n instanceof ImplicitParameterNode) objects.add(n);
    }
    ...
    while (iter.hasNext()) {
      ...
      if (e instanceof CallEdge) {
        Node end = e.getEnd();
        if (end instanceof CallNode) ((CallNode) end).setSignaled(((CallEdge) e).isSignal());
      }
    }
    ...
    while (iter.hasNext()) {
      Node n = (Node) iter.next();
      if (n instanceof CallNode) top = Math.max(top, n.getBounds().getY() + n.getBounds().getHeight());
    }
  ...
  public void draw(Graphics2D g2, Grid g) {
    ...
    while (iter.hasNext()) {
      Node n = (Node) iter.next();
      if (!(n instanceof CallNode)) n.draw(g2);
    }
